...
public abstract class List extends Dictionary {
    ...
    // Can be called with any node in the DLL. So this function should search
    ...
    // Delete searches for the node with the same key and same d and returns true
    // only if d as well as d.key match
    ...
    // returns an element with key >= k in the dictionary. Returns null in case no
    // such element found.
    ...
    // In this implementation getFirst() returns the first element of the list and
    ...
    // dictionary are to be traversed.
    // The only requirement is that using the following loop, getFirst() and
    // getNext() should be able to traverse all the elements in the dictionary.
